...
### combine_and_export
Объединяет все добавленные фигуры в один .vox файл.

**Параметры:**
- `filename` (string): Имя итогового файла
- `output_directory` (string, опционально): Директория для сохранения файла (по умолчанию: текущая директория)
- `merge_strategy` (enum): "single_object" (одна модель) или "scene_graph" (отдельные модели)

**Логика:** Сервер сопоставляет Hex-цвета с палитрой, рассчитывает пересечения вокселей и формирует иерархию чанков.
